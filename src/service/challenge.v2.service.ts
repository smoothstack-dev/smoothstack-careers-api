...
import { ChallengeEventV2, ChallengeSession } from 'src/model/ChallengeEvent';
...
export const generateChallengeLink = async (submission: JobSubmission): Promise<string> => {
  ...
  const invitation = {
    ...
    test_result_url: `${CALLBACK_URL}?provider=hackerrank&submissionId=${submissionId}`,
  };
...
export const processChallengeEvent = async (
  { score, max_score, plagiarism_status }: ChallengeEventV2,
  submissionId: number
) => {
